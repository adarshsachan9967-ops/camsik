...
import { Eye, EyeOff, Phone, Mail, User, MapPin, Truck, FileText, ChevronRight, ArrowLeft, CheckCircle } from 'lucide-react';
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-primary/5 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Your delivery agent application is under review. Our team will verify your documents and activate your account within <strong>24–48 hours</strong>.
          </p>
          <div className="bg-gray-50 rounded-2xl p-4 text-left mb-6 space-y-2">
            ...
            {['Document verification by our team', 'Background & vehicle check', 'Account activation via SMS', 'Start accepting delivery tasks'].map((step, i) => (
          ...
          <div className="p-6">
            {mode === 'signin' ? (
              <>
                ...
                <p className="text-gray-500 text-sm mb-6">Sign in to your delivery agent account</p>
                <form onSubmit={handleSignIn} className="space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Phone Number</label>
                    <div className="relative">
                      <Phone size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="tel" value={siPhone} onChange={e => setSiPhone(e.target.value)} required
                ...
                <div className="mt-5 bg-green-50 rounded-2xl p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-600">Flexible hours · Weekly payouts · Fuel allowance</p>
                  </div>
                </div>

                <p className="text-center text-xs text-gray-400 mt-5">
                  New agent?{' '}
                  <button onClick={() => setMode('signup')} className="text-primary font-bold hover:underline">Register here</button>
                </p>
              </>
            ) : (
              <>
